...
type PersistedWorkerRecord = Omit<WorkerRecord, "status">;

const DEFAULT_OFFLINE_MS = 45_000;

export class WorkerRegistry {
  private readonly workers = new Map<string, PersistedWorkerRecord>();
  ...
  async listWorkers() {
    ...
    return [...this.workers.values()]
      ...
      .sort((left, right) => (
        workerStatusOrder(left.status) - workerStatusOrder(right.status)
  ...
  async register(input: unknown) {
    ...
    const worker: PersistedWorkerRecord = {
      id,
  ...
  async heartbeat(workerId: string, input: unknown) {
    await this.init();
  ...
  private snapshot(worker: PersistedWorkerRecord): WorkerRecord {
...
export function ensureWorkerAuthorized(req: IncomingMessage, res: ServerResponse) {
...
function normalizePersistedWorker(value: unknown): PersistedWorkerRecord | null {
  if (!isRecord(value)) {
    return null;
  }
...
function normalizeWorkerPool(value: unknown, fallback: string) {
...
function workerStatusOrder(status: WorkerStatus) {
...
function sendWorkerAuthError(res: ServerResponse, statusCode: number, error: string) {
