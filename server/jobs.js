...
import { loadServerConfig, normalizeJobDefaults } from "./config";
import { AUDIT_ROOT, DEFAULT_JOB_LOG_TAIL, JOBS_ROOT, OUTPUT_ROOT, SOURCES_ROOT, UPLOADS_ROOT, WORKER_RESULTS_ROOT, } from "./platform";
...
export class JobManager {
    ...
    async getJob(id, tailLines = DEFAULT_JOB_LOG_TAIL) {
        await this.init();
        await this.recoverExpiredWorkerLeases();
    ...
    async continueWithFocus(runId, additionalRounds = 2) {
        ...
        const focusSuggestions = Array.isArray(focusState?.suggestions)
            ...
            : [];
        const resumableFocusCount = focusSuggestions.filter((suggestion) => (suggestion.status === "queued" ||
            suggestion.status === "scheduled" ||
            suggestion.status === "running")).length;
        ...
        const focusExecutions = Array.isArray(focusState?.executions)
            ...
            : [];
        const usedFocusRounds = new Set(focusExecutions
        ...
        const maxFocusRounds = boundedInteger(focusState?.maxFocusRounds, 0, 12, job.settings.maxFocusRounds);
    ...
    async resumeFromStage(runId, stage, requestedRound) {
        await this.init();
        ...
        await this.appendLog(job.id, `Queued checkpointed resume from ${stage}${resumeRound === null ? "" : ` in round ${resumeRound}`}.`);
        void this.kickQueue();
        return updated;
    }
    async adoptExistingRun(runId) {
    ...
    async createUploadJob(formData) {
        await this.init();
        const config = await loadServerConfig();
    ...
    async leaseWorkerJob(workerId, pool) {
        await this.init();
        while (this.leasePromise) {
            await this.leasePromise;
        }
        let releaseLeaseLock = () => { };
        this.leasePromise = new Promise((resolve) => {
    ...
    async heartbeatWorkerJob(workerId, jobId, leaseId, stage) {
        await this.init();
    ...
    async appendWorkerLog(workerId, jobId, leaseId, lines) {
        await this.init();
    ...
    async getWorkerSourceArchive(workerId, jobId, leaseId) {
        await this.init();
        const job = this.assertWorkerLease(workerId, jobId, leaseId);
    ...
    async createWorkerResumeArchive(workerId, jobId, leaseId) {
        await this.init();
        const job = this.assertWorkerLease(workerId, jobId, leaseId);
    ...
    async installWorkerResult(workerId, jobId, leaseId, archive) {
        await this.init();
        const job = this.assertWorkerLease(workerId, jobId, leaseId);
    ...
    async installWorkerWorkspace(workerId, jobId, leaseId, archive) {
        await this.init();
        const job = this.assertWorkerLease(workerId, jobId, leaseId);
        if (!isUploadFile(archive)) {
    ...
    async completeWorkerJob(workerId, jobId, leaseId, exitCode = 0) {
        await this.init();
        const job = this.assertWorkerLease(workerId, jobId, leaseId);
        ...
        await this.appendLog(job.id, awaitingReview
    ...
    async failWorkerJob(workerId, jobId, leaseId, error, exitCode = null) {
        await this.init();
        const job = this.assertWorkerLease(workerId, jobId, leaseId);
    ...
    assertWorkerLease(workerId, jobId, leaseId) {
    ...
    async recoverExpiredWorkerLeases() {
        const expired = [...this.jobs.values()].filter((job) => (job.settings.executionMode === "worker"
    ...
    async initialize() {
        ...
        for (const entry of jobFiles) {
            try {
                const raw = JSON.parse(await fs.readFile(path.join(JOBS_ROOT, entry.name), "utf-8"));
    ...
    async executeJob(jobId) {
        ...
        try {
            ...
            const exitCode = await this.runLoggedCommand(jobId, pythonCommand, buildAuditCommandArgs(targetDir, job.outputDir, job.settings, job.projectId, job.engagementId, resumeRequested, job.resumeFromStage, job.resumeRound), {
                cwd: AUDIT_ROOT,
            ...
            await this.appendLog(jobId, awaitingReview
    ...
    async prepareWorkspace(jobId) {
        ...
        if (job.source.type === "github") {
            ...
            await this.runLoggedCommand(jobId, "git", ["clone", "--depth", "1", job.source.repoUrl, job.workspacePath], { cwd: AUDIT_ROOT, timeoutMs: 20 * 60 * 1000 });
            if (job.source.ref) {
                await this.appendLog(jobId, `Checking out ref ${job.source.ref}`);
                await this.runLoggedCommand(jobId, "git", ["fetch", "--depth", "1", "origin", job.source.ref], { cwd: job.workspacePath, timeoutMs: 10 * 60 * 1000 });
                await this.runLoggedCommand(jobId, "git", ["checkout", "FETCH_HEAD"], { cwd: job.workspacePath, timeoutMs: 10 * 60 * 1000 });
            }
        ...
        if (uploadPath.endsWith(".zip")) {
            await this.runLoggedCommand(jobId, "unzip", ["-q", uploadPath, "-d", job.workspacePath], { cwd: AUDIT_ROOT, timeoutMs: 20 * 60 * 1000 });
        }
        else if (uploadPath.endsWith(".tar.gz") || uploadPath.endsWith(".tgz") || uploadPath.endsWith(".tar")) {
            await this.runLoggedCommand(jobId, "tar", ["-xf", uploadPath, "-C", job.workspacePath], { cwd: AUDIT_ROOT, timeoutMs: 20 * 60 * 1000 });
        }
        else {
    ...
    async runLoggedCommand(jobId, command, args, options) {
        await this.ensureNotCancelled(jobId);
    ...
    async ensureNotCancelled(jobId) {
        const job = this.jobs.get(jobId);
        if (job?.cancelRequested) {
...
function normalizeGithubJobInput(value, defaults) {
    if (!isRecord(value)) {
...
function normalizeFormSettings(formData, defaults) {
...
function buildAuditCommandArgs(targetDir, outputDir, settings, projectId, engagementId, resume = false, resumeFromStage = null, resumeRound = null) {
    const args = [
...
async function detectTargetRoot(extractedRoot) {
...
async function readTail(filePath, tailLines) {
    try {
...
async function installTarArchive(archive, archivePath, stagingPath, destinationPath, maximumBytes) {
